...
function calculateMaintainabilityIndex(code1) {
  const code = removeComments(code1);
  // Hitung jumlah baris kode (tanpa baris kosong)
  const linesOfCode = code.split('\n').filter((line) => line.trim().length > 0).length;

  // Hitung jumlah komentar (baris komentar satu baris dan multi-line)
  const singleLineComments = code.split('\n').filter((line) => line.includes('//')).length;
  const multiLineComments = (code.match(/\/\*[\s\S]*?\*\//g) || []).length;
  const commentLines = singleLineComments + multiLineComments;

  // Persentase komentar dalam kode
  const perCM = (commentLines / linesOfCode) * 100;
  ...
  const n1 = new Set(operators).size; // Operator unik
  const n2 = new Set(operands).size; // Operand unik
  const N1 = operators.length; // Operator total
  const N2 = operands.length; // Operand total

  // Volume Halstead
  const halsteadVolume = (N1 + N2) * Math.log2(n1 + n2);

  console.log('Halstead Vol ' + halsteadVolume);
  console.log('Operator ' + operators);
  console.log('Operand ' + operands);

  console.log('Operator unik ' + n1);
  console.log('Operand unik ' + n2);
  console.log('Operator total ' + N1);
  console.log('Operand total ' + N2);

  console.log('CC' + cyclomaticComplexity);

  // Hitung Maintainability Index (MI)
  let maintainabilityIndex = 171 - 5.2 * Math.log(halsteadVolume) - 0.23 * cyclomaticComplexity - 16.2 * Math.log(linesOfCode);

  // Pastikan MI tidak lebih kecil dari 0
  maintainabilityIndex = Math.max(maintainabilityIndex, 0);

  // Tentukan warna dan penjelasan berdasarkan MI
  let color = 'Green';
  let explanation = 'This code is highly maintainable.';
  if (maintainabilityIndex <= 65) {
    color = 'Red';
...
// Fungsi untuk menghapus komentar dari kode
function removeComments(code) {
  // Menggunakan regex untuk menghapus komentar satu baris (//) dan komentar multi-baris (/*...*/)
  return code.replace(/\/\/[^\n]*|\/\*[\s\S]*?\*\//g, '');
}
...
export const detectDataClassSmell = (req, res) => {
  try {
    ...
    // Variabel untuk menghitung jumlah file yang mengandung kelas yang terdeteksi
    let dataClassDetectedFilesCount = 0;
    ...
    const dataClassResults = phpFiles
      .map(({ fileName, code }) => {
        ...
        if (!classMetrics) {
          // Abaikan file tanpa metrik kelas
          return null;
        }
        ...
        // Jika terdeteksi sebagai Data Class Smell, tambahkan jumlah file yang mengandung kelas
        if (className) {
          dataClassDetectedFilesCount++;
        }
    ...
    // Menampilkan jumlah file yang mengandung kelas yang terdeteksi
    console.log(`Number of files with detected Data Class Smell: ${dataClassDetectedFilesCount}`);

    res.json({
      success: true,
      data: dataClassResults,
      fileWithDataClassSmellCount: dataClassDetectedFilesCount, // Include count in the response
    });
  } catch (error) {
    console.error(error);
    res.status(500).json({
